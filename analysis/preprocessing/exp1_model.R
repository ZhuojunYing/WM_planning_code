# ==============================================================================
# Experiment 1 Simulation Preprocessing
# Author: WM Planning manuscript team
# Purpose: Merge planning and exposure simulations, calculate reward metrics,
#          and attach relational rewards for the 6-node network.
# ==============================================================================
...
df_plan <- read_analysis_csv("data", "raw_data", "exp1_model.csv") %>%
  dplyr::mutate(condition = "planning")
print(length(unique(df_plan$graph)))
...
df_clean <- df %>%
  dplyr::mutate(
    ...
    parent_reward = ifelse(
      node %in% c(2, 3, 5, 6),
      path_reward - actual_reward,
      0
    ),
...
# Saving Outputs
# ==============================================================================
df_test <- df_clean %>% filter(condition == "planning")
print(mean(df_test$accuracy))
write_analysis_csv(df_clean, "data", "processed_data", "exp1_model.csv")
